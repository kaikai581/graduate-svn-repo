...
PrintAdPmtAlg::PrintAdPmtAlg(const string& name, ISvcLocator* svcloc) :
GaudiAlgorithm(name, svcloc)
{
}


StatusCode PrintAdPmtAlg::execute()
{
...
StatusCode PrintAdPmtAlg::finalize()
{
...
StatusCode PrintAdPmtAlg::initialize()
{
  ...
  info() << pmtPos[0] << " " << pmtPos[1] << " " << pmtPos[2] << endreq;
  const Hep3Vector& pmtPosl = m_pmtGeomSvc->get(pmtSensors[6].fullPackedData())->localPosition();
  info() << pmtPosl[0] << " " << pmtPosl[1] << " " << pmtPosl[2] << endreq;
  
  //info() << m_pmtGeomSvc->get(pmtSensors[6].fullPackedData())->detectorElement().name() << endreq;
  //Gaudi::XYZPoint Origin = m_pmtGeomSvc->get(pmtSensors[6].fullPackedData())->detectorElement().geometry()->toGlobal(Gaudi::XYZPoint(0,0,0));
