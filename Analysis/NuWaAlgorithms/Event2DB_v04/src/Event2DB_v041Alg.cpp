...
#define DATABASE "EventDisplayDB_v041"
...
Event2DB_v041Alg::Event2DB_v041Alg(const string& name, ISvcLocator* svcloc) :
GaudiAlgorithm(name, svcloc), m_updatecount(0), m_exeCntr(1)
{
}


StatusCode Event2DB_v041Alg::initialize()
{
  string url(DBHOST);
...
StatusCode Event2DB_v041Alg::finalize()
{
...
StatusCode Event2DB_v041Alg::execute()
{
  info() << "execute()" << endreq;
...
void Event2DB_v041Alg::printRuntimeErr(std::runtime_error& e)
{
...
void Event2DB_v041Alg::printSqlErr(SQLException& e)
{
...
void Event2DB_v041Alg::processAd(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
  ...
  for(channelIter=channels.begin();channelIter!=channelDone;++channelIter)
  {
    ...
    insstr << "INSERT IGNORE INTO adPmtCalibStats (detectorTriggerId, adPmtId, ";
    insstr << "totCharge)";
...
void Event2DB_v041Alg::processMuon()
{
...
void Event2DB_v041Alg::processRpc(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
  const Context& context = m_calibRoHdr->context();
  ...
  for(channelIter=channels.begin();channelIter!=channelDone;++channelIter)
  {
    ...
    insstr << "INSERT IGNORE INTO rpcStripCalib (detectorTriggerId, rpcStripId)";
    insstr << " SELECT " << triggerId;
    insstr << ", rpcStripId";
...
void Event2DB_v041Alg::processTrigger()
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
...
void Event2DB_v041Alg::processWs(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
  const Context& context = m_calibRoHdr->context();
  ...
  for(channelIter=channels.begin();channelIter!=channelDone;++channelIter)
  {
    ...
    insstr << "INSERT IGNORE INTO wsPmtCalibStats (detectorTriggerId, wsPmtId, ";
    insstr << "totCharge)";
    insstr << " SELECT " << triggerId;
    insstr << ", wsPmtId";
    insstr << ", " << channel.sumCharge();
