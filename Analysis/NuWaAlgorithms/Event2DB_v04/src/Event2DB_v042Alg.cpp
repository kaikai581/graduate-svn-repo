...
#define DATABASE "EventDisplayDB_v042"
...
Event2DB_v042Alg::Event2DB_v042Alg(const string& name, ISvcLocator* svcloc) :
...
StatusCode Event2DB_v042Alg::initialize()
{
  string url(DBHOST);
...
StatusCode Event2DB_v042Alg::finalize()
{
...
StatusCode Event2DB_v042Alg::execute()
{
  info() << "execute()" << endreq;
...
void Event2DB_v042Alg::printRuntimeErr(std::runtime_error& e)
{
...
void Event2DB_v042Alg::printSqlErr(SQLException& e)
{
...
void Event2DB_v042Alg::processAdCalib(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
...
void Event2DB_v042Alg::processAdRec(int triggerId)
{
  ...
  insstr << ", x = " << recAdTrig.position().x() + gblOrigin.X();
  insstr << ", y = " << recAdTrig.position().y() + gblOrigin.Y();
  insstr << ", z = " << recAdTrig.position().z() + gblOrigin.Z();
...
void Event2DB_v042Alg::processMuon()
{
...
void Event2DB_v042Alg::processRpcCalib(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
  const Context& context = m_calibRoHdr->context();
...
void Event2DB_v042Alg::processRpcRec(int triggerId)
{
...
void Event2DB_v042Alg::processTrigger()
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
...
void Event2DB_v042Alg::processWsCalib(int triggerId)
{
  const CalibReadout* calibReadout = m_calibRoHdr->calibReadout();
  const Context& context = m_calibRoHdr->context();
...
void Event2DB_v042Alg::processWsRec(int triggerId)
{
  ...
  insstr << " SET detectorTriggerId = " << triggerId;
  insstr << ", x = " << recWsTrig.position().x() + gblOrigin.X();
  insstr << ", y = " << recWsTrig.position().y() + gblOrigin.Y();
  insstr << ", z = " << recWsTrig.position().z() + gblOrigin.Z();
  insstr << ";";
