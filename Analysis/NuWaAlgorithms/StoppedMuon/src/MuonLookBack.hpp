...
private:
  ...
  double                              getIavTrackLength(MyDataModel::AdTrigger&);
  ...
  double        m_windowEnd;
  double        m_trackCylinderRadius;
  
  /// muon track and IAV intersection properties
  Gaudi::XYZPoint m_pInIav;
  Gaudi::XYZPoint m_pOutIav;
  double          m_totCylLen;
  double          getCylLen();
  int             isInCyl(const MyDataModel::RpcTrigger&, const MyDataModel::AdTrigger&, const MyDataModel::AdTrigger&);
  /// variables and methods for analytic method of inscribing cylinder
  bool            circleInsideIav(TrackInfo&, double);
