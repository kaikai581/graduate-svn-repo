...
double ChargeEnergyRatio(TFile& f, int tn, map<int, vector<pair<float, float> > >& hits)
{
  ...
  for(unsigned int i = 0; i < muTrigNum.size()-1; i++)
  {
    ...
    for(int nli = 1; nli < nlist; nli++)
    {
      ...
      if(tEv - tMu > 1e-3)
      {
        ...
        /// get the charge energy ratio
        double ceratio = ChargeEnergyRatio(inf, rh->triggerNumber, pmtHits);
