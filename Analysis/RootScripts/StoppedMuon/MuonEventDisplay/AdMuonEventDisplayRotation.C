...
/*
 * It's highly probably that the reconstructed points are 180 degree rotated
 * along z with respect to the PMTs when transforming to global coordinate.
 * This script will rotate the reconstructed points by 180 degree before
 * rendering.
 */
...
int AdMuonEventDisplayRotation(int coordtype = 0, int detector = 1, int trNum = 1441)
{
  if(detector > 4)
  {
  ...
  if(coordtype == 0)
  {
    ...
    AxisAngle rotPiAlongZ(XYZVector(0,0,1),TMath::Pi());
    pRec = rotPiAlongZ * pRec;
