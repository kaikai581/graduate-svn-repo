...
void SeaLevelEnergyVsAngleCompDavid()
{
  ...
  c1->SetGridx(1);
  TH2F* h = new TH2F("h", "sea level data", 100,-50,2200,100,0,46);
  h->SetStats(0);
  ...
  for(int h = 0; h < 3; h++)
  {
    ...
    g[h]->SetLineWidth(2);
    g[h]->SetTitle("sea level data");
    g[h]->GetXaxis()->CenterTitle();
    g[h]->GetXaxis()->SetTitle("mean muon energy (GeV)");
    g[h]->Draw("PZ");
  }
  ...
  c1->SaveAs("figures/CompDavid_sea.eps");
}
