...
string getOutputPathName(string ifpn, string ofp, string treename)
{
  string ofpn = "", ofn;
  ...
  {
    cerr << "specified path doesn't exist" << endl;
    cerr << "will be outputting to current directory instead" << endl;
    ofpn = "./" + ofn;
  }
...
{
  if(ifpn.rfind("_") == string::npos)
    return string("");
  return ifpn.substr(ifpn.rfind("_")+1, 4);
}
