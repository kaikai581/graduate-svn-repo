...
OUTFILEPATH=/eliza7/dayabay/scratch/sklin/RootFile/RpcProductionTree/$YEAR/$VER/Neutrino/$DATE
...
do
  if grep -q ${fn} ${LISTDIR}/NoRpcFileList.txt; then
    ...
    OUTFILENAME=rpctree.$OUTFILENAME
    if [ ! -f $OUTFILEPATH/$OUTFILENAME ]; then
      qsub -V -l eliza16io=1 -l eliza7io=1 execute.sh ${WORKDIR} ${fn} ${OUTFILEPATH}&
    else
