...
CrossDetector/OfficialProduction/src");
  ...
  ofn = "eff." + ofn;
  string ofpn = ofp + ofn;
  cout << "processing file: " << endl << ofpn << endl;
...
//              GetValuePointer();
      ...
      for(unsigned int nm = 0; nm < nModules; nm++)
      {
        ...
        for(unsigned int ns = 0; ns < nStripThisMod[nm]; ns++)
        {
          if(ns == 0)
          {
            hitModRow.push_back(rh->rpcRow[ns]);
            hitModCol.push_back(rh->rpcColumn[ns]);
          }
          layerThisMod.push_back(rh->rpcLayer[ns]);
          getHit[rh->rpcLayer[ns]-1] = true;
          stripThisMod.push_back(rh->rpcStrip[ns]);
        }
